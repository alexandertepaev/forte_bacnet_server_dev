...
CBacnetAnalogOutputObject::CBacnetAnalogOutputObject(TForteUInt32 paObjectID, float paPresentValue, bool paCOVReportingEnabled, float paCOVIncrement, CBacnetObjectConfigFB *paConfigFB) : CBacnetCOVReportingObject(OBJECT_ANALOG_OUTPUT, paObjectID, paCOVReportingEnabled, paConfigFB), mPresentValue(paPresentValue), mCOVIncrement(paCOVIncrement) {
...
int CBacnetAnalogOutputObject::readProperty(TForteUInt8 *paBuffer,  BACNET_PROPERTY_ID paProperty) {
  int len = 0;
  switch (paProperty)
  {   
...
void CBacnetAnalogOutputObject::encodeValueList(BACNET_PROPERTY_VALUE* paValueList) {
  //present value
