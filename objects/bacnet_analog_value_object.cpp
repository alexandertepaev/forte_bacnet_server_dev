...
CBacnetAnalogValueObject::CBacnetAnalogValueObject(TForteUInt32 paObjectID, float paPresentValue, bool paCOVReportingEnabled, float paCOVIncrement, CBacnetObjectConfigFB *paConfigFB) : CBacnetCOVReportingObject(OBJECT_ANALOG_VALUE, paObjectID, paCOVReportingEnabled, paConfigFB), mPresentValue(paPresentValue), mCOVIncrement(paCOVIncrement) {
...
int CBacnetAnalogValueObject::readProperty(TForteUInt8 *paBuffer,  BACNET_PROPERTY_ID paProperty) {
  int len = 0;
  switch (paProperty)
  {   
...
void CBacnetAnalogValueObject::encodeValueList(BACNET_PROPERTY_VALUE* paValueList) {
  //present value
