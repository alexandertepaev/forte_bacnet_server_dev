...
CBACnetAnalogInputObject::CBACnetAnalogInputObject(uint32_t paObjectID, float paPresentValue, bool paCOVReportingEnabled, float paCOVIncrement, forte::core::io::IOConfigFBBase *paConfigFB) : CBacnetCOVReportingObject(OBJECT_ANALOG_INPUT, paObjectID, paCOVReportingEnabled, paConfigFB), mPresentValue(paPresentValue), mCOVIncrement(paCOVIncrement) {
...
int CBACnetAnalogInputObject::readProperty(uint8_t *buffer,  BACNET_PROPERTY_ID property) {
  int len = 0;
  switch (property)
  {   
...
void CBACnetAnalogInputObject::encodeValueList(BACNET_PROPERTY_VALUE* value_list) {
  //present value
